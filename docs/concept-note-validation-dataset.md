...
**Technical annex to the concept note on S.1503-4 validation material — draft, 24 September 2026**
...
4. **The elevation floor is not verifiable from a range-shaped mask by inspection.** Near-peak power thins with slant range before it reaches low elevations, with or without a floor, and the lit reach (power within 20 dB of the block peak) sits at 2 to 3 degrees on every range-shaped low-orbit mask in the corpus whether or not a boresight floor gated the beams — measured on 20 September 2026: the saturated masks and their floor-gated controls light the ground to the same elevation. The check therefore reports the lit reach beside the elevation grade and does not grade on it, and its elevation grade must not be read as a verified floor.
5. **Coarse time steps understate the maxima.** On the 1 200 km family system the maxima rose about 20 dB from 60 s to 6 s steps and 0.2 to 0.4 dB more at 1 s. On the constructed 1 600-satellite system over half a day the truth’s maximum rose 2.0 to 3.3 dB from 60 s to 6 s at 0 to 50 degrees, and the gap between the examination and the truth closed by 0.5 to 2.9 dB at 0 to 40 degrees. The examination at 1 s reads within 0.1 dB of the time step the Recommendation prescribes (§ D4: a 0.208 s fine step and a 4.16 s coarse step), and at 6 s within 0.5 dB. Body percentiles are stable from 60 s. With a random satellite selection and no hold the step is also the reselection period, so a finer step is also a faster traffic model. The truth now runs on a preset 1 s step, and every figure names its step beside its depth. Tail levels are quotable only at the step that resolves them.
...
7. **The gap between the examination and the truth is the envelope’s, not the selection rules’.** The margin decomposition runs three curves on one victim grid and one comb of 60 s steps — the truth, the examination’s selection rules (§ D5.1.4.1: elevation gate, exclusion zone, the MAX_CO_FREQ pick, MIN_ANGLE_AT_ES pruning, the main-beam always-include) applied to the truth’s own values, and the examination proper over the declared mask. On the constructed 1 600-satellite system the selection component is 0.0 dB at the deciding point of every latitude from 0 to 60 at 0.5 and 1.0 days (0.0 to −0.1 at 0.1 days) and removes 0.2 to 0.7 dB in the body; the envelope component is the whole gap, +7.5 to +11.6 dB by latitude at 0.5 days (+7.5 to +11.0 at 1.0, +6.5 to +12.1 at 0.1). The selection the examination substitutes for the operator’s scheduler costs nothing where the verdict is decided; the price of the format is the per-direction maximum, and every granularity lever acts on that component. The zero of the decomposition is a checked invariant (with nothing declared the selection curve equals the truth bin for bin), and its first record had to be withdrawn: one live read shared across victims re-entered the scheduler out of order, which the instrument now forbids. At those depths the 60 s step under-samples the truth more than the examination, so the envelope component there is overstated by up to about 3 dB where the maximum decides (item 5); the selection component’s zero is a comparison at matched step and stands.
8. **A convergence pair firms the body of a curve and only provisionally its tail.** The truth-only sweep of the constructed system was extended on one comb of 60 s steps from 0.5 to 1.0, 2.0, 4.0, 8.0 and 16.0 days. At each doubling one or two latitudes moved by 1 to 3 dB at the deciding point while the others stood, and the latitudes that stood over one doubling were not the ones that stood over the next: a maximum can only rise under extension, a resolved percentile can fall as rarer samples dilute a cluster. The examination followed the same geometry through the mask, so the gap between the two moved less than either curve (6.4 to 10.6 dB at 0 to 40 degrees at every depth from 0.5 days) and the examination stayed at or above the truth at every latitude at every depth. Consequence for the dataset: an expectation curve’s body is quotable from its pair; its short-term end is quotable only with the run length that produced it, whatever that run length is. Much of the movement was the 60 s step being paid down by depth: at 16 days the examination sits within 0.4 dB of the examination on the Recommendation’s time step over half a day, and the truth within 0.8 dB of the truth at 6 s over half a day, except at 30 degrees (1.6 dB) (item 5).
9. **The verdict is the limit curve, not its tabulated points.** Article 22 tabulates pairs of epfd and percentage of time; between adjacent pairs the limit is the log-linear interpolation, and the gaps are wide (5.4, 10.0 and 18.0 dB for the 0.70 m victim of Table 22-1C, 34.4 dB for the 2.5 m one). A distribution can clear both ends of a gap and cross the curve inside it. The examination as the Bureau runs it on filed cases fails such a distribution; two independent point-wise implementations pass it. The design brief therefore adopted one verdict rule on 18 September: pass only if every tabulated point passes and the distribution nowhere crosses the interpolated curve, with a stated tolerance (half a result bin, read towards lower epfd) and the worst crossing reported. Laid over this dataset’s expectations the rule flipped four expected verdicts and took the read-rule probe BL-R1’s discrimination away at the verdict level; both read-rule probes were re-tuned by measurement and re-emitted, every record now names the rule it verdicts under, and the producer’s own verdict path applies it. Consequence for an implementation: a point-wise test is the lenient reading and must not be assumed by default.
10. **The victim’s geometry is worth about 2 dB to the examination on this system, and the projection gap does not depend on it.** The records plant the victim at one geometry (earth station at longitude 0, wanted GSO satellite 10 degrees east); the Recommendation’s worst-case geometry search is the consumer’s. Moving the GSO satellite over eight longitude offsets from 10 degrees west to 60 east, at every latitude from 0 to 60, on 60 s steps, moved the truth’s worst margin by 3.3 to 14.9 dB per latitude and the examination’s by 4.1 to 12.2, but the gap between them by only 2.6 to 3.9 dB, and the examination stayed at or above the truth at all 98 victims of both depths. The default offset is the worst for the examination at 30 and 50 degrees and within 0.6 dB of the worst at 10, 20, 40 and 60 at 2.0 days; at the equator the satellite overhead reads 2.1 dB worse on the examination and 4.1 dB worse on the truth, a difference that stood from 0.5 to 2.0 days, while a 5 dB difference at 40 degrees at 0.5 days collapsed to 0.6 dB at 2.0 as the baseline’s own maximum rose. The one positive truth margin ever recorded, +1.3 dB at the equator with the satellite 5 degrees east at 0.5 days, read −4.4 at 2.0 days and failed the limit curve at both. Measured as a sensitivity over a coarse grid, the geometry component is a lower bound: about 2 dB at the equator, 0.6 dB or less elsewhere at 2.0 days, against an envelope component of 7 to 11 dB. At 2.0 days on 60 s steps the examination reads within 0.9 dB of the fine sampling, so the differences between offsets are matched-step comparisons; the absolute margins carry the step (item 5).

### 4.5 A first use outside the dataset

The producer's examination was used in September by the two sessions drafting the concept note on No. 11.32A for non-GSO systems. Its case (a) — applying the Article 22 values of an adjacent band to a non-GSO system with a geostationary victim, so that an operator who protects the arc has a route to a favourable finding — rested on an unmeasured claim. Measured on the constructed system under the limit-curve verdict rule (re-measured 18 September; the point-wise figures of 14 September were 2 to 4 dB more lenient): a system whose mask carries its declared zone clears the 22-1C rows at a boresight pfd of about −120 to −126 dB(W/(m² MHz)) depending on the victim dish, a working downlink level; the same payload with the arc lit would have to operate 18 to 32 dB lower. The protector is limited at the body of the row, the non-protector at the short-term end by the single main-beam pass inside the zone; the criterion bites hardest near the interfering shell's own inclination, where its satellites are densest — a property an examiner can anticipate from the filing. The note now carries these as measured statements with their caveats, and the same records serve the Bureau's implementation.
...
4. **The consistency check's elevation axis.** Settled on 21 September 2026: a lit-reach criterion was measured against every mask in the corpus before it was built and re-grades floor-gated and floor-less low-orbit masks alike, so the lit reach is reported beside the grade and not graded (§ 4.4, item 4). Still open: whether the Bureau's own implementation should carry the check at all, which is a question for the -4 plan.
...
**Project records:** the dataset design brief and the consumer guide (implementation project repository, `architecture/`); `dataset/*/expected/` and `dataset/*/README.md`; `docs/simulation-debate.md`; `docs/construction.html`; `docs/arc-shield-case-a.md`; the loop records `docs/compliance-steam-2*.md` (the depth ladder at 0.1, 0.5, 1.0, 2.0, 4.0, 8.0 and 16.0 days on 60 s steps; half a day on a 6 s step, `-6s`, and with the examination also on the S.1503-4 time step, `-d4step`; the co-frequency beam capacity runs, cap 4 at 0.1, 0.5 and 1.0 days and caps 12 and 20 at 0.1 days; the GSO-offset runs `-gso*`, seven offsets besides the default at 0.5 days and five of them also at 2.0 days; and one examination of a filed mask) and the decomposition records `docs/margin-decomposition-steam-2*.md` (0.1, 0.5 and 1.0 days).
