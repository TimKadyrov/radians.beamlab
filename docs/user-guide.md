...
- **Co-channel sum, K-colour reuse** — assumes a frequency plan: the
  ...
  *Where K comes from:* the PFD mask is an operator-declared envelope of the
  system's actual co-frequency operation (Rec. ITU-R S.1503-4, Annex 1
  Part C), so the reuse factor is **your system design choice, not a
  regulatory value** — set it to match the payload's real channelisation, or
  use the power sum as the K = 1 worst case. The K = 3/4/7 colourings are
  the classical hexagonal reuse clusters (N = i² + ij + j²: MacDonald, Bell
  Syst. Tech. J., 1979; Maral & Bousquet, *Satellite Communications
  Systems*); reuse-1 vs reuse-3 are exactly the two options studied for NTN
  in 3GPP TR 38.821.
...
- **Rec. ITU-R S.1503-4** — mask coordinates, the α avoidance angle and its
  sign (§D6.4.4), the satellite az/el frame (§D6.4.5), the mask XML schema,
  and the principle that the PFD mask is generated by the operator to
  envelope actual co-frequency operation (Annex 1 Part C).
- **Frequency reuse** — hexagonal cluster theory (N = i² + ij + j²):
  V. H. MacDonald, "Advanced Mobile Phone Service: The Cellular Concept",
  Bell Syst. Tech. J. 58(1), 1979; Maral & Bousquet, *Satellite
  Communications Systems*; 3GPP TR 38.821 (NTN, FRF = 1 and FRF = 3).
