# Compliance loop: STEAM-2-gsoE20 (STEAM-2 profile unchanged; GSO satellite +20 deg east of the earth station, 0.5 d, declaration reused from the record)

*Produced by `dotnet run --project tests/radians.beamlab.checks -- loop "STEAM-2-gsoE20.opprofile.json" "STEAM-2.orbitdesign.json" 0.5 60 0 60 10 reuse=dataset/margin/steam-2 gso=20`.*
*Date: 2026-09-21. Wall clock 4.2 min.*
...
- Victim: earth station at longitude 0, GSO satellite +20 deg, dish 1.00 m (the limit row's own reference diameter).
...
|---|---|---|---|---|---|---|---|
| 0 | -151.9 | -12.2 | 0.029% | -14.2 | crosses at -164.1 dB: 0.9722% vs 0.03252% allowed (limit 0.0313%), x29.90 | FAIL | 0 |
| 10 | -154.5 | -9.7 | 0.029% | -11.7 | crosses at -164.1 dB: 0.5556% vs 0.03252% allowed (limit 0.0313%), x17.09 | FAIL | 0 |
| 20 | -155.2 | -9.0 | 0.029% | -11.0 | crosses at -164.1 dB: 0.2778% vs 0.03252% allowed (limit 0.0313%), x8.54 | FAIL | 0 |
| 30 | -156.8 | -7.3 | 0.029% | -9.3 | crosses at -164.1 dB: 0.6944% vs 0.03252% allowed (limit 0.0313%), x21.36 | FAIL | 0 |
| 40 | -157.7 | -8.5 | 0.286% | -10.1 | crosses at -164.1 dB: 0.9722% vs 0.03252% allowed (limit 0.0313%), x29.90 | FAIL | 0 |
| 50 | -157.8 | -9.8 | 1% | -10.9 | crosses at -164.1 dB: 1.389% vs 0.03252% allowed (limit 0.0313%), x42.71 | FAIL | 0 |
| 60 | -157.6 | -9.5 | 1% | -9.7 | crosses at -164.1 dB: 1.806% vs 0.03252% allowed (limit 0.0313%), x55.53 | FAIL | 0 |
...
**EXCEEDED at 7 of 7 latitude(s) (0, 10, 20, 30, 40, 50, 60); worst margin -14.2 dB under the limit-curve rule -- sampled every 10 deg over 0..60, so a finer sweep can find worse between them -- power headroom -14.2 dB on per-beam TxEirpDbw (dB-for-dB)**
...
|---|---|---|---|---|---|---|
| 0 | -12.2 | -18.6 | 6.4 | 0.029% / 0.029% | -14.2 / -20.6 | yes |
| 10 | -9.7 | -17.3 | 7.6 | 0.029% / 0.029% | -11.7 / -19.3 | yes |
| 20 | -9.0 | -17.7 | 8.7 | 0.029% / 0.029% | -11.0 / -19.7 | yes |
| 30 | -7.3 | -15.9 | 8.6 | 0.029% / 0.029% | -9.3 / -17.9 | yes |
| 40 | -8.5 | -18.2 | 9.7 | 0.286% / 0.029% | -10.1 / -20.2 | yes |
| 50 | -9.8 | -23.0 | 13.2 | 1% / 1% | -10.9 / -23.4 | yes |
| 60 | -9.5 | -21.4 | 11.9 | 1% / 1% | -9.7 / -21.6 | yes |

Margins and the gap in this table are point-wise; the curve margins beside them are the rule's second test.

**ADEQUATE: E1 >= T at every latitude; widest gap 13.2 dB**

## Artefacts

The profile is this run's; the R set and the pfd mask are the reused run's, copied here so the directory stands alone:

- Operation profile (the truth as run): `dataset\margin\steam-2-gsoe20\steam-2-gsoe20.opprofile.json`
- Reused R set (S.1503-4 Part B): `dataset\margin\steam-2-gsoe20\steam-2-gsoe20.operparams.xml`
- Reused R set, designer format (open with the operating-parameters designer): `dataset\margin\steam-2-gsoe20\steam-2-gsoe20.operparams.json`
- Declared pfd mask: `dataset\margin\steam-2-gsoe20\steam-2-gsoe20.mask.reused.xml`
