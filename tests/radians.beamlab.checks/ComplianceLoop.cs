...
internal static class ComplianceLoop
{
        ...
        string? reuseDir = null, double gsoOffset = 10.0, double esLon = 0.0, bool d4Exam = false)
    {
        ...
        foreach (var r in rows)
            Console.WriteLine(string.Create(inv,
                $"{r.LatDeg,4:F0} | {r.MaxEpfdDb,9:F1} | {r.WorstMarginDb,+9:F1} | {r.CurveMarginText,9} | {(r.Pass ? "PASS" : "FAIL"),4} | {r.QuietSteps}"));
        ...
        sb.AppendLine();
        sb.AppendLine("| latitude | max epfd (dB) | point margin (dB) | deciding point (% of time) | curve margin (dB) | curve crossing | verdict | quiet steps |");
        sb.AppendLine("|---|---|---|---|---|---|---|---|");
        foreach (var r in rows)
            sb.AppendLine(string.Create(inv, $"| {r.LatDeg:F0} | {r.MaxEpfdDb:F1} | {r.WorstMarginDb:+0.0;-0.0} | {r.DecidingText} | {r.CurveMarginText} | {r.CrossingText} | {(r.Pass ? "PASS" : "FAIL")} | {r.QuietSteps} |"));
        sb.AppendLine();
        sb.AppendLine(LimitCurveRule.Name());
        sb.AppendLine();
        sb.AppendLine("The deciding point is the limit point at the worst point margin: \"max\" is the maximum sample against the 0% row, which can only rise as the run lengthens; a percentage names a resolved percentile, which can move either way.");
        sb.AppendLine();
        ...
        else
        {
            ...
            sb.AppendLine();
            sb.AppendLine("| latitude | T margin (dB) | E1 margin (dB) | gap (dB) | deciding point T / E1 | curve margin T / E1 (dB) | E1 >= T |");
            sb.AppendLine("|---|---|---|---|---|---|---|");
            for (int i = 0; i < rowsE1.Count; i++)
                sb.AppendLine(string.Create(inv,
                    $"| {rows[i].LatDeg:F0} | {rows[i].WorstMarginDb:+0.0;-0.0} | {rowsE1[i].WorstMarginDb:+0.0;-0.0} | "
                    + $"{rows[i].WorstMarginDb - rowsE1[i].WorstMarginDb:F1} | {rows[i].DecidingText} / {rowsE1[i].DecidingText} | {rows[i].CurveMarginText} / {rowsE1[i].CurveMarginText} | {(rowsE1[i].WorstMarginDb <= rows[i].WorstMarginDb + 1e-9 ? "yes" : "**NO**")} |"));
            sb.AppendLine();
        ...
        double latStep, string tag, bool d4Exam = false)
    {
        ...
        sb.AppendLine("|---|---|---|---|---|---|---|");
        foreach (var r in rows)
            sb.AppendLine(string.Create(inv, $"| {r.LatDeg:F0} | {r.MaxEpfdDb:F1} | {r.WorstMarginDb:+0.0;-0.0} | {r.DecidingText} | {r.CurveMarginText} | {r.CrossingText} | {(r.Pass ? "PASS" : "FAIL")} |"));
        sb.AppendLine();
        sb.AppendLine(LimitCurveRule.Name());
        sb.AppendLine();
        if (rowsD4 is not null && planD4 is not null)
