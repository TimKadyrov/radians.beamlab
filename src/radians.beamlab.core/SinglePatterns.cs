...
public interface ISinglePattern
{
    ...
    /// <summary>
    /// Gain for an off-axis angle theta and an azimuth phi around the boresight (degrees).
    /// Circular patterns ignore phi. Elliptical patterns (Rec1528_1p4_Ell) use it.
    /// phi = 0 is the radial axis (Lr direction); phi = 90 deg is the transverse axis (Lt direction).
    /// </summary>
...
///
/// F(u) = [2*J1(pi*u) / (pi*u)] * Prod_{n=1..n_bar-1} (1 - u^2/u_n^2) / (1 - u^2/mu_n^2)
/// G(theta) = Gm + 20*log10|F(u)|,   u = u_edge * sin(theta) / sin(theta_b)
///
/// where mu_n = j_{1,n}/pi are the un-modified Bessel zeros, u_n =
/// sigma*sqrt(A^2 + (n-1/2)^2) are the Taylor replacement zeros, A = arccosh(10^(SLR/20))/pi,
/// and sigma = mu_{n_bar}/sqrt(A^2 + (n_bar-1/2)^2). u_edge is solved numerically so that
/// |F|^2 = 1/2 at theta = theta_b.
/// </summary>
public sealed class Rec1528_1p4 : ISinglePattern
{
    ...
    // First six positive zeros of J1, divided by pi to give the un-modified Taylor mu_n.
    ...
    /// <summary>The "u" coordinate corresponding to the half-3-dB beamwidth (where |F|^2=1/2).</summary>
    ...
    {
        // |F| is positive and monotone-decreasing on [0, mu_1) starting at F(0)=1.
        // Bisect for F(u) = 1/sqrt2.
...
///
/// <code>
///   u = sqrt( (Lr*sintheta*cosphi)^2 + (Lt*sintheta*sinphi)^2 ) / lambda
/// </code>
///
/// phi = 0 is the radial axis (Lr direction, along the off-nadir tilt for a typical
/// LEO non-GSO beam); phi = 90 deg is the transverse axis (Lt direction). For Lr = Lt
...
/// the half-3-dB beamwidth on the radial cut (phi = 0); for the transverse cut it is
/// theta_b * Lr/Lt.
/// </summary>
public sealed class Rec1528_1p4_Ell : ISinglePattern
{
    ...
    private readonly double _uHalf;   // u value at which |F|^2 = 1/2
                           ...
                           double slrDb = 20.0, int nbar = 4, double lfDbi = 0.0)
    {
        ...
        // Half-3-dB beamwidth per principal cut: |F(u_half)|^2 = 1/2 =>
        //   radial: u = (Lr/lambda)*sintheta => theta_b_rad = asin(u_half * lambda / Lr)
        //   transverse: theta_b_tr = asin(u_half * lambda / Lt)
    ...
    /// <summary>
    /// Convert an Annex-2-style cell description (half-axis angles alpha, beta subtended at the
    /// satellite, edge-of-cell roll-off in dB) into Lr, Lt in metres, given lambda.
...
///
/// <code>
///   G1 = Gmax - 3*(psi/psib)^1.5         0 <= psi &lt; a*psib
///   G2 = Gmax + LN                   a*psib <= psi &lt; b*psib
///   G3 = Gmax + LN - 25*log10(psi/(b*psib))  b*psib <= psi &lt; Y
///   G4 = LF                          Y <= psi &lt; 90 deg
///   G5 = max(15 + LN + 0.25*Gmax, 0)  90 deg <= psi <= 180 deg    (Table 1 z=1; for LN=-15 this simplifies to 0.25*Gmax)
///   G  = MAX(G1, G2, G3, G4, G5)
/// </code>
///
/// Constants from Table 1 (z=1): a = 2.58, b = 6.32. <c>Y = b*psib*10^(0.04*(Gmax + LN - LF))</c>.
/// </summary>
public sealed class Rec1528_1p2 : ISinglePattern
{
    ...
    public double LF { get; }
    public double LB { get; }   // Back-lobe floor (90 deg..180 deg)
    ...
    public double Gain(double thetaDeg)
    {
        ...
        // clamped so log10 >= 0: below b*psib the formula extrapolates to +inf,
        // so we plateau it at the boundary value Gm + LN. G5 is gated to
        // psi >= 90 deg so the back-lobe doesn't apply on the forward hemisphere.
...
///
/// <code>
///   G1 = Gmax - 3*(psi/psib)^1.5     0 <= psi &lt; psib
///   G2 = Gmax - 3*(psi/psib)^2       psib <= psi &lt; Y
///   G3 = Gmax + Ls - 25*log10(psi/Y)  Y <= psi &lt; Z
///   G4 = LF                       Z <= psi &lt; 180 deg
///   G  = MAX(G1, G2, G3, G4)
/// </code>
///
...
/// <list type="bullet">
///   <item>LEO: Ls = -6.75 dB, Y = 1.5*psib</item>
///   <item>MEO: Ls = -12 dB,    Y = 2*psib</item>
///   <item>HEO: Ls = -20 dB,    Y = psib*sqrt(-Ls/3) = psib*sqrt(20/3) ~ 2.582*psib</item>
/// </list>
...
public sealed class Rec1528_1p3 : ISinglePattern
{
    ...
    public double Gain(double thetaDeg)
    {
        ...
        // log10 >= 0 so it plateaus at Gm + Ls below psi = Y instead of
        // extrapolating to +inf near the boresight.
...
{
    /// <summary>S.1528-1 Sec. 1.4 Taylor circular illumination.</summary>
    Taylor_1p4,
    /// <summary>S.1528-1 Sec. 1.4 Taylor elliptical illumination (Lr != Lt).</summary>
    Taylor_1p4_Ell,
    /// <summary>S.1528-0 Sec. 1.2 envelope (APSREC409V01-style).</summary>
    Envelope_1p2,
    /// <summary>S.1528-0 Sec. 1.3 LEO (APSREC411V01).</summary>
    Leo_1p3,
    /// <summary>S.1528-0 Sec. 1.3 MEO (APSREC410V01).</summary>
    Meo_1p3,
    /// <summary>S.1528-0 Sec. 1.3 HEO (APSREC4XXV01).</summary>
    Heo_1p3,
}
