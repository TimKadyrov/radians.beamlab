...
public static class ParameterCatalog
{
    ...
    public static readonly IReadOnlyList<ParameterInfo> All = new ParameterInfo[]
    {
        ...
        new(ParameterGroup.Truth, "IlluminationDutyCycle", "(0,1]",
            ...
            "Beam-hopping time average for frames much shorter than the time step: every resolved beam power carries 10 log₁₀(duty). The declared masks stay peak-PSD envelopes — only the simulated statistics average (see the duty row in the Activity timeline).",
            new[]
            {
