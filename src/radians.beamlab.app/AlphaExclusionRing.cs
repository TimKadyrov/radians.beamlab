...
/// <summary>
/// One concentric GSO-exclusion ring, defined by its outer α edge (deg). Rings
/// stack from α = 0 outward; ring i covers [previous outer, this outer). A
/// beam whose footprint |α| falls in the ring is either switched off or
...
public sealed class AlphaExclusionRing : ObservableObject
{
    ...
    /// <summary>Outer α boundary of the ring (deg from the nearest visible GSO arc).</summary>
...
/// the outer α edge, whether it switches beams off, and the attenuation (dB)
/// otherwise. Basic single-threshold mode reduces to one off band at α_excl.
/// </summary>
